...
Encoder.encode = function(route, msg){
	if(!route || !msg){
		logger.warn('Route or msg can not be null! route : %j, msg %j', route, msg);
		return null;
	}
	...
	if(!checkMsg(msg, protos)){
		logger.warn('check msg failed! route: %j msg : %j, proto : %j', route, msg, protos);
		return null;
	}
...
function checkMsg(msg, protos){
	if(!protos || !msg){
		logger.warn('no protos or msg exist! msg : %j, protos : %j', msg, protos);
		return false;
	}
		...
		switch(proto.option){
			case 'required' :
				if(typeof(msg[name]) === 'undefined'){
					logger.warn('no property exist for required! name: %j, proto: %j, msg: %j', name, proto, msg);
					return false;
				}
			case 'optional' :
				if(typeof(msg[name]) !== 'undefined'){
					...
					if(!!message && !checkMsg(msg[name], message)){
						logger.warn('inner proto error! name: %j, proto: %j, msg: %j', name, proto, msg);
						return false;
					}
				}
			break;
		...
		switch (proto.type) {
			case 'uInt32': {
				...
				if (value < 0 || value > 0xffffffff) {
					logger.warn('value overflow error! type: %s, value: %j, name: %j, proto: %j, msg: %j', proto.type, value, name, proto, msg);
					return false;
				}
			}
			break;
			...
			case 'sInt32': {
				const value = msg[name];
				if (value < -2147483648 || value > 0x7fffffff) {
					logger.warn('value overflow error! type: %s, value: %j, name: %j, proto: %j, msg: %j', proto.type, value, name, proto, msg);
					return false;
				}
			}
